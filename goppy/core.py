...
class OnlineGP(object):
	...
	Provides a Gaussian process to which further data can be added 
        efficiently after the initial training.
	...
	expected_size : int, optional
		The overall expected number of training samples to be added to
                the Gaussian process. Setting this parameter can be more
                efficient as it may avoid memory reallocations.
	...
	noise_var : float
		The assumed variance of the noise on the training targets.
	x_train : (`N`, `D`) ndarray
		The `N` training data inputs of dimension `D`. This will be
                ``None`` as long as the Gaussian process has not been trained.
	y_train : (`N`, `D`) ndarray
		The `N` training data targets of dimension `D`. This will be
                ``None`` as long as the Gaussian process has not been trained.
	...
	trained : bool
		Indicates whether the Gaussian process has been fitted to some
                training data.
	...
	def inv_cov_matrix(self):
		...
		if self.__inv_cov_matrix is None:
			self.__inv_cov_matrix = np.dot(
                                self.inv_chol.T, self.inv_chol)
	...
	def fit(self, x, y):
		...
		y : (`N`, `D`) array-like
			The `N` training targets with `D` independent
                        dimensions.
		"""
		...
		if self._expected_size is not None:
			...
			buffer_shape2 = (
                                self._expected_size, self._expected_size)
		else:
			buffer_shape = buffer_shape2 = None

		self.x_train = self._buffer_factory(
                        x.shape, buffer_shape=buffer_shape)
		self.y_train = self._buffer_factory(
                        y.shape, buffer_shape=buffer_shape)
		self.x_train[:, :] = x
		self.y_train[:, :] = y
	...
	def add(self, x, y):
		"""Adds additional training data to the Gaussian process and
                adjusts the fit.
		...
		covmat[diag_indices] = np.maximum(
                        self.noise_var, covmat[diag_indices])
	...
	def calc_log_likelihood(self, what=('value',)):
		r"""Calculate the log likelihood or its derivative of the
                Gaussian process.

		Depending on the values included in the `what` parameter
                different values will be calculated:

		* ``'value'``: The log likelihood of the Gaussian process as
                  scalar.
		* ``'derivative'``: Partial derivatives of the log likelihood
                  for each kernel parameter as array. See the ``params``
                  property of the used kernel for the order.
		...
		if 'derivative' in what:
			...
			res['derivative'] = np.array([
				0.5 * np.sum(np.einsum(
                                    ...
                                    self.inv_cov_matrix, param_deriv))
				for param_deriv in self.kernel.full(
					self.x_train, self.x_train,
                                        what='param_derivatives')[
                                            'param_derivatives']])
		return res
	...
	def predict(self, x, what=('mean',)):
		...
		Depending on the values included in the `what` parameter
                different predictions will be made and returned a dictionary
                ``res``:

		* ``'mean'``: Mean prediction of the Gaussian process of shape
                  (`N`, `D`).
		* ``'mse'``: Predictive variance of the Gaussian process of
                  shape (`N`,).
		* ``'derivative'``: Predicted derivative of the mean.
		  ...
		  \left(x_i\right)` with the `i`-th input data point
                  :math:`x_i`, and mean function :math:`\mu(x)`.
		* ``'mse_derivative'``: Predicted derivative of the variance.
		  ...
		  \left(x_i\right)` with the `i`-th input data point
                  :math:`x_i`, and variance function :math:`\sigma^2(x)`.

		Parameters
		----------
		x : (`N`, `D`) array-like
			The `N` data points of dimension `D` to predict data
                        for.
		what : set-like, optional
		...
		lazy_vars = _LazyVarCollection(
			...
			mean=lambda v: np.dot(
                            v.input_vs_train_dist['y'], v.svs),
			mse_svs=lambda v: np.dot(
				self.inv_cov_matrix,
                                v.input_vs_train_dist['y'].T),
			mse=lambda v: np.maximum(
				self.noise_var,
				self.noise_var + self.kernel.diag(x, x) -
                                np.einsum(
					'ij,ji->i',
                                        v.input_vs_train_dist['y'], v.mse_svs)),
			derivative=lambda v: np.einsum(
				'ijk,jl->ilk',
                                v.input_vs_train_dist['derivative'], v.svs),
			mse_derivative=lambda v: -2 * np.einsum(
				'ijk,ji->ik',
                                v.input_vs_train_dist['derivative'], v.mse_svs))
